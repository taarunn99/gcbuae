...
export function TerrazzoFinishExplorer() {
  const [active, setActive] = useState(0);

  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_1.2fr] lg:items-center">
      <div>
        <div role="tablist" aria-label="Terrazzo surface finishes">
          {FINISHES.map((f, i) => (
            <button
              key={f.slug}
              role="tab"
        ...
        <p className="text-ink/50 mt-5 text-sm">
          Polished is standard; Honed 400 carries the R9 slip rating (DIN
          51130).
        </p>
      </div>
