...
## Scroll film (home page)

- `FilmSequence` is a single pinned GSAP master timeline (400vh), not
  per-section triggers. Curtain transitions and overlays are scrubbed and
  reversible; the four videos play once, forward only, and hold their end
  frame. Never tie video currentTime to scroll, and never re-play on reverse.
- Video files are generated by `npm run assets:film`
  (`scripts/build-film-assets.sh`) from gitignored masters in
  `assets/source/film/` (CloudFront URLs in the script). Sources are 1284×716 —
  do not upscale in ffmpeg.
- Inside a GSAP timeline, a `fromTo` positioned later does NOT render its
  from-state on creation — set initial states with `gsap.set` up front, or
  layers stack visibly broken.
- The header hides during the pin via the `data-film-pinned` attribute on
  `<html>`; the CSS lives in globals.css under `.site-header`.
